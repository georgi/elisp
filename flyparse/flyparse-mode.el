...
(defvar flyparse-no-changes-timeout 0.2
...
(defvar flyparse-error-in-last-parse nil
  "Did the parser process signal an error in the last parser.")
...
(defun flyparse-for-each-cached-tree (func &optional file-pattern)
  "Call 'func' for each tree in 'flyparse-tree-cache'."
    ...
    (flyparse-walk-path
     ...
     (lambda (ea-dir ea-file)
	   ...
	   (let* ((file-path (expand-file-name (concat ea-dir ea-file)))
		  (tree (flyparse-block-and-load-tree-from-file file-path cmd-list)))
	     (if tree
		 (progn
  ...
  (when (memq (process-status process) '(signal exit))
    (let* ((exit-status       (process-exit-status process))
	  ...
	  (progn
		...
		;; Process exited with status=0, but still
		;; printed something to stderr..
		(if (> (length stderr) 0)
		    (flyparse-log 1 stderr)))))
	(error
	 (let ((err-str (format "Error in process sentinel for buffer %s: %s: %s"
				source-buffer (error-message-string err) stderr)))
	   (flyparse-cleanup-temp-files)
	   (setq flyparse-error-in-last-parse t)
	   (flyparse-report-status "Error" "*")
	   (flyparse-log 0 err-str)))))))


(defun flyparse-can-parse-file (file-name)
  "XXX: At some point, this should check if parsing is supported
   for the file-type."
  t)
...
(define-minor-mode flyparse-mode
  ...
  (cond
   ...
   ;; Turning the mode ON.
   (flyparse-mode
    (if (not (flyparse-can-parse-file buffer-file-name))
	(flyparse-log 2 "Flyparse cannot parse file in buffer %s." (buffer-name))
...
(defun flyparse-run-tests ()
  ...
  (let* ((tree 
	 ...
	 (result (flyparse-query-all '("ROOT" ("SUBTREE" (where ("SUBTREE" "APPLE"))) "LEAF") tree)))
  ...
  (let* ((tree 
	  '("ROOT" (0 23)
	    ...
	    ("SUBTREE" (3 20)
	     ("MINI" (0 10))
	     ("MINI" (0 7)))))
	 (result (flyparse-search '("SUBTREE" (where ("SUBTREE" "MINI"))) tree)))
  ...
  (let* ((tree 
	  '("ROOT" (0 23)
	    ...
	    ("SUBTREE" (3 20)
	     ("MINI" (0 10))
	     ("MINI" (0 7)))))
	 (result (flyparse-directed-search '("SUBTREE" (where ("SUBTREE" "MINI"))) 17 tree)))
    (assert (equal result '("SUBTREE" (6 23) ("MINI" (0 10))("MINI" (0 7))))))
...
(defun flyparse-block-and-load-tree-from-file (file-path cmd-list)
  ...
  (let ((cmd (mapconcat 'identity (append cmd-list (list file-path)) " ")))
    (let* ((result-string (with-output-to-string
      ...
      (assert (listp result-form) t 
	      "Attempted to load a tree but found something else.")
      (eval (first (rest (rest result-form)))))))
